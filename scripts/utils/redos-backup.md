...
Скрипт работает в интерактивном режиме: пользователь, режим копирования,
накопитель и имя папки выбираются в процессе запуска.
...
Для полного бэкапа HOME расчёт размера использует те же исключения, что и последующее копирование через `rsync`.
...
* автоматическую обработку символических ссылок с учётом файловой системы накопителя
* пропуск устройств, сокетов и других спецфайлов, которые нельзя корректно восстановить на обычном USB-накопителе
* устойчивость к ошибкам
* возможность восстановления после прерывания

Имена папок назначения проверяются: нельзя указать пустое имя, `.`/`..` или путь со слешем. Это защищает от случайной записи вне выбранного накопителя.
