#!/bin/bash
##############################################################################
# install-1c.sh — Установка 1С:Предприятие на РЕД ОС 7.3/8.0
#
# Описание: Установка платформы 1С:Предприятие версии 8.3.24.1691
#
# Использование:
#   sudo ./install-1c.sh [ОПЦИИ]
#
# Опции:
#   -h, --help           Справка
#   -w, --work-dir DIR   Рабочая директория (по умолчанию /tmp/1c)
#   -t, --tag TAG        Версия релиза (по умолчанию — последний GitHub Release)
#   -v, --version VER    Версия 1С (по умолчанию 8.3.24.1691)
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -t|--tag)
            ...
            ;;
        -v|--version)
            ONEC_VERSION="${2:?Ошибка: требуется значение для --version}"
            shift 2
            ;;
        *)
