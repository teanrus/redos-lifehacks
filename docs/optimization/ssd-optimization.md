...
> ⚠️ **Важно про discard:**  
>  
> Параметр `discard` в `fstab` может снижать производительность на некоторых SSD. Рекомендуется использовать периодический TRIM через `fstrim` вместо постоянного `discard`.
