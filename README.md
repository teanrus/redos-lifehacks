...
- 🔌 [peripheral](/docs/peripheral/readme.md) — периферия: принтеры, сканеры, серверы печати, смарт-карты, RFID, сканеры штрих-кодов
- 📊 [monitoring](/docs/monitoring/readme.md) — мониторинг: здоровье системы, логи, совместимость оборудования
...
- 🧹 [utils](/scripts/utils/readme.md) — вспомогательные скрипты (очистка системы, управление сохранёнными паролями, монтирование сетевых шар, блокировка USB)
...
Подробная таблица отличий версий 7.x от 8.x: ядро, рабочие столы, пакеты, безопасность, сроки поддержки, руководство по обновлению
...
### ⏰ [redos-auto-update](scripts/monitoring/redos-auto-update.md)

Автоматическое обновление системы по расписанию: настраиваемое временное окно (например, 12:30–14:00), режимы безопасности/полное/проверка, гибкое расписание, уведомления MAX Messenger

### 🖥️ [mount-manager](scripts/utils/mount-manager.md)

Управление монтированием сетевых шар (CIFS/SMB): интерактивное меню, пресеты подключений, автоматическое добавление в fstab, поддержка доменных шар

### 🔒 [usb-guard](scripts/utils/usb-guard.md)

Управление блокировкой USB-накопителей: методы UDISKS_IGNORE и authorized, белый список доверенных устройств, udev-правила
